  ...
  appendRow(isDown) {
    ...
    while (allColIds.size > 0) {
      ...
      foreachTr.foreachCellInner((cell) => {
        ...
        if (cell.colspan > 1) {
          let index = colIds.indexOf(cell.colId);
          if (index === -1) {
            index = 0;
          }
  ...
  */
  removeRow() {
    ...
    for (const tr of rows) {
      tr.foreachCellInner((cell) => {
        const rowIndexInSelected = rowIds.indexOf(cell.rowId);
  ...
  appendCol(isRight) {
    ...
    if (isRight) {
      ...
      const i = colIds.indexOf(lastTd.colId);
    ...
    for (const tr of rows) {
      ...
      tr.foreachCellInner((cell) => {
        ...
        const colIndexInSelected = colIds.indexOf(cell.colId);
  ...
  removeCol() {
    ...
    const [firstSelectColIndex, lastSelectColIndex] = tds.reduce(
      (n, cellInner) => {
        const i = colIds.indexOf(cellInner.colId);
